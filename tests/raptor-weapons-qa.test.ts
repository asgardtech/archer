...
describe("Laser Weapon", () => {
  ...
  test("Laser with rapid-fire increases tick rate to 18 ticks/sec", () => {
    const beam = new LaserBeam();
    beam.active = true;
    beam.setModifiers(true, false); // rapidFire = true

    // Tick interval = 1/18 ≈ 0.0556s
    const tick1 = beam.update(0.05);
    expect(tick1).toBe(false);
...
describe("Power-Up Drop Configuration (Levels)", () => {
  ...
  test("Level 2 only drops missiles", () => {
    ...
    expect(level2.weaponDrops).toEqual(["missile"]);
  });
  ...
  test("Level 3 drops missile and laser", () => {
  ...
  test("Level 7 through 10 have the full late-game arsenal", () => {
  ...
  test("All levels from 3 onward include missile and laser", () => {
...
describe("Weapon Balance Validation", () => {
  test("Missile homing strength is reduced to 1.8", () => {
    const missile = WEAPON_CONFIGS["missile"];
    expect(missile.homingStrength).toBe(1.8);
  });

  test("Auto-gun homing strength is 1.3", () => {
    expect(WEAPON_CONFIGS["auto-gun"].homingStrength).toBe(1.3);
  });

  test("Auto-gun tier 2 has increased damage multiplier of 1.2", () => {
    const autogun = WEAPON_CONFIGS["auto-gun"];
    expect(autogun.tiers[1].damageMultiplier).toBe(1.2);
  });

  test("Plasma has larger splash radius than missile", () => {
  ...
  test("Plasma fire rate is 0.7", () => {
    expect(WEAPON_CONFIGS["plasma"].fireRateMultiplier).toBe(0.7);
  });

  test("Rocket has the largest splash radius of all weapons", () => {
  ...
  test("Rocket fire rate is 0.3", () => {
    expect(WEAPON_CONFIGS["rocket"].fireRateMultiplier).toBe(0.3);
  });

  test("Ion cannon charge time is 1.6", () => {
    expect(WEAPON_CONFIGS["ion-cannon"].chargeTime).toBe(1.6);
  });

  test("Ion cannon tier 2 damage multiplier is 1.5", () => {
    expect(WEAPON_CONFIGS["ion-cannon"].tiers[1].damageMultiplier).toBe(1.5);
  });

  test("Laser rapid-fire bonus is 1.8", () => {
    expect(WEAPON_CONFIGS["laser"].rapidFireBonus).toBe(1.8);
  });

  test("Machine gun tier 3 fires 3 projectiles with spread 0.1", () => {
    const mg = WEAPON_CONFIGS["machine-gun"];
    expect(mg.tiers[2].projectileCount).toBe(3);
    expect(mg.tiers[2].projectileSpread).toBe(0.1);
  });

  test("Each splash weapon has a configured splash damage ratio", () => {
    for (const [, cfg] of Object.entries(WEAPON_CONFIGS)) {
  ...
  test("No weapon has the highest value in all categories simultaneously", () => {
    ...
    for (const w of weapons) {
      const hasAll = w.damage === maxDamage &&
  ...
  test("Homing weapons have lower or equal average raw DPS than non-homing weapons", () => {
    const autoFireRate = 5;
    function rawDps(cfg: typeof WEAPON_CONFIGS["machine-gun"]): number {
      if (cfg.fireRateMultiplier === 0) return cfg.damage * 10;
      return cfg.damage * autoFireRate * cfg.fireRateMultiplier;
    }

    const homingWeapons = [WEAPON_CONFIGS["missile"], WEAPON_CONFIGS["auto-gun"]];
    const nonHomingWeapons = [WEAPON_CONFIGS["plasma"], WEAPON_CONFIGS["rocket"]];

    const avgHomingDps = homingWeapons.reduce((s, w) => s + rawDps(w), 0) / homingWeapons.length;
    const avgNonHomingDps = nonHomingWeapons.reduce((s, w) => s + rawDps(w), 0) / nonHomingWeapons.length;

    expect(avgHomingDps).toBeLessThanOrEqual(avgNonHomingDps);
  });

  test("All weapon types have complete WEAPON_CONFIGS entries", () => {
    expect(Object.keys(WEAPON_CONFIGS)).toHaveLength(7);
    const allTypes: (keyof typeof WEAPON_CONFIGS)[] = [
    ...
    for (const t of allTypes) {
      ...
      expect(typeof cfg.projectileSpeed).toBe("number");
      expect(typeof cfg.piercing).toBe("boolean");
      expect(typeof cfg.homing).toBe("boolean");
      expect(typeof cfg.homingStrength).toBe("number");
      expect(typeof cfg.splashRadius).toBe("number");
      expect(typeof cfg.splashDamageRatio).toBe("number");
      expect(typeof cfg.rapidFireBonus).toBe("number");
